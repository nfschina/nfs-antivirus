...
*/
	...
	printf("file_info is :%s\n",file_info);
    if((pstatusFile = fopen(file_info,"abr+")) == NULL)
    {
            return ;
    }
...
//                fprintf(avl_status_file,"%s\n",szAvlStatus);
    fclose(pstatusFile);
    delete_log_30daysago(file_info);
    system("chmod -R  777 /usr/share/antivirus");
...
int main(int argc,char *argv[])
{
	...
	if(argc == 2)
	{
	     if(strcmp(argv[1],"") != 0)
		 {
			snprintf(g_current_user,sizeof(g_current_user),"%s",argv[1]);
		 }

	}
#endif

    system("rm -f /var/log/clamav/freshclam.log");
    ...
    strcpy(cmdBuf, "last -n 1 |awk '{print $1}'|head -1");
    stream = popen(cmdBuf, "r");
    fread(recvBuf, sizeof(char), sizeof(recvBuf)-1, stream);
    pclose(stream);
    printf("recvBuf is :%s\n",recvBuf);
    if(strlen(recvBuf) > 0)
    {
        strncpy(g_current_user,recvBuf,sizeof(g_current_user));
    }
    printf("g_current_user is :%s\n",g_current_user);
    g_current_user[strlen(g_current_user) - 1] = '\0';
    ...
    snprintf(upgrade_log_temp_file,100,"%s.%s_avlUpgradeLog_tmp.txt",ANTIVIRUS_TEMP_DIR,g_current_user);
    ...
    write_upgrade_log(upgrade_log_file,szLoginfo);
    write_upgrade_log(upgrade_log_temp_file,szLoginfo);
    system("chmod 777 /usr/share/antivirus/log/avlUpgradeLog.txt");
